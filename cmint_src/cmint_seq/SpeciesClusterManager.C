...
/*
CMINT: An algorithm to cluster functional omics data from multiple cell types on a lineage
...
SpeciesClusterManager::initExperts()
{
	for(map<string,CLUSTERSET*>::iterator aIter=speciesExpertSet.begin();aIter!=speciesExpertSet.end();aIter++)
	{
		...
		string srcSpeciesStr(srcSpecies);
		for(CLUSTERSET_ITER cIter=speciesExpert->begin();cIter!=speciesExpert->end();cIter++)
		{
			...
			//estimateMeanCov_insideout(e,(string&)aIter->first,cIter->first);
			estimateMeanCov_insideout(e,srcSpeciesStr,cIter->first);
		}
...
SpeciesClusterManager::estimateExpertParameters(const char* outputDir)
{
	...
	//int nMaxIter=1;
	char llFName[1024];
	sprintf(llFName,"%s/likelihood.txt",outputDir);
	ofstream llFile(llFName);
	while((!convergence)&&(iter<nMaxIter))
	//while((!convergence)&&(iter<2))
	{
		expectationStep();
		////Getting rid of hack
		maximizationStep();
		//This is a hack, we need to fix this. I am hardcoding here and saying if maxIter=1, we don't want to learn but just estimate the gammas
		/*if(nMaxIter>1)
		{
			maximizationStep();
		}*/
		...
		{
			char subdir[1024];
			sprintf(subdir,"%s/iter%d",outputDir,iter);
			char command[1024];
			sprintf(command,"mkdir %s/iter%d",outputDir,iter);
			system(command);
			//dumpAllInferredClusterAssignments(outputDir,iter);
			gammaMgr->getAllClusterAssignments(mappedClusterAssignment,true);
			showClusters_Extant_Partial(subdir);
		}*/
		...
		llFile<<"Iter: " << iter << " score: " << newScore << " diff " << diff << endl;
		currScore=newScore;
		iter++;
	}
	if(convergence)
...
SpeciesClusterManager::getScore()
{
	...
	for(map<string,CLUSTERSET*>::iterator cIter=speciesExpertSet.begin();cIter!=speciesExpertSet.end();cIter++)
	{
		...
		cout <<"Getting score contrib of " << cIter->first << endl;
...
int
SpeciesClusterManager::estimateMeanCov_insideout(Expert* e, string& specName, int clusterID)
{
	GeneExpManager* exprMgr=speciesExprSet[specName];
	map<string,int>* speciesGenes=speciesClusterSet_Genewise[specName];
	vector<double>* expr=exprMgr->getExp(speciesGenes->begin()->first);
	...
	double totaldp=0;
	map<int,double> meanstore;
	map<int,double> sumstore;
	//initialize all dims
	for(int d=0;d<dim;d++)
	{
		meanstore[d]=0;
	...
	for(map<string,int>::iterator dIter=speciesGenes->begin();dIter!=speciesGenes->end();dIter++)
	{
		//Assuming that each evidence is actually a joint assignment to RVs. What we
		//want is a mean and variance for each experiment
		vector<double>* geneExpr=exprMgr->getExp(dIter->first);
		if(geneExpr==NULL)
		{
		...
		int ogid=mor->getMappedOrthogroupID((string&)dIter->first,(string&)specName);
		//Matrix* gamma_i_k_s=gammaMgr->getGamma(ogid,(string&)dIter->first,specName);
		Matrix* norm_i_k_s=gammaMgr->getNormTerm(ogid,(string&)dIter->first,specName);
		if(norm_i_k_s==NULL)
		{
			norm_i_k_s=gammaMgr->getGamma(ogid,(string&)dIter->first,specName);
		}
		else if(norm_i_k_s->getColCnt()==1)
		{
			norm_i_k_s=gammaMgr->getGamma(ogid,(string&)dIter->first,specName);
		}
		//double sum=0;
		...
		for(int d=0;d<dim;d++)
		{
			...
			/*for(int r=0;r<gamma_i_k_s->getRowCnt();r++)
			{
				...
				//meanval=meanval+(g_i*eval);
				meanstore[d]=meanstore[d]+(g_i*eval);
				sumstore[d]=sumstore[d]+g_i;
				origval=origval+(g_i*eval);		
				colsum=colsum+g_i;
			}*/
			if(norm_i_k_s==NULL)
			{
				cout <<"Fatal error! No normalization term initialized" << endl;
				exit(0);
			}
			double n_i=norm_i_k_s->getValue(0,clusterID);
			double newval=eval*n_i;
			meanstore[d]=meanstore[d]+(newval);
			sumstore[d]=sumstore[d]+n_i;
		}
	}
	for(int d=0;d<dim;d++)
	{
		double sum=sumstore[d];
		double meanval=meanstore[d];
		if(sum==0)
		{
			cout <<"No members in cluster " << clusterID << endl;
		}
	...
	/*for(map<string,int>::iterator dIter=speciesGenes->begin();dIter!=speciesGenes->end();dIter++)
	{
		...
		//Matrix* gamma_i_k_s=gammaMgr->getGamma(ogid,(string&)dIter->first,specName);
		Matrix* norm_i_k_s=gammaMgr->getNormTerm(ogid,(string&)dIter->first,specName);
		//So if this is the root, then we need to set norm_i_k_s to be gamma_i_k_s
		if(norm_i_k_s==NULL)
		{
			norm_i_k_s=gammaMgr->getGamma(ogid,(string&)dIter->first,specName);
		}
		...
		for(int i=0;i<dim;i++)
		{
			...
			for(int j=i;j<i+1;j++)
			{
				...
				//{
				//	double g_i=gamma_i_k_s->getValue(r,clusterID);
				//	sumstore[i]=sumstore[i]+g_i;
				//	double diff=g_i*((*geneExpr)[i]-m1)*((*geneExpr)[j]-m2);
				//	covstore[i]=covstore[i]+diff;
				//}
				double n_i=norm_i_k_s->getValue(0,clusterID);
				double diff=n_i*((*geneExpr)[i]-m1)*((*geneExpr)[j]-m2);
				sumstore[i]=sumstore[i]+n_i;
				covstore[i]=covstore[i]+diff;
			}
		}
	}*/
	for(int i=0;i<dim;i++)
	{
		//covstore[i]=covstore[i]/sumstore[i];
		//covariance->setValue(covstore[i],i,i);
		covariance->setValue(0.1,i,i);
		//covariance->setValue(0.2,i,i);
	//	covariance->setValue(covstore[i],j,i);
	}
	//cout <<"Mean estimated from "<< totaldp << " in " << specName <<":" << clusterID<< endl;
//	mean->showMatrix();
	if(e->getMean()==NULL)
...
int
SpeciesClusterManager::showClusters_Extant_Partial(const char* outputDir)
{
	char aFName[1024];
	assignGenesToExperts_FromMap();
	for(map<string,CLUSTERSET*>::iterator sIter=speciesExpertSet.begin();sIter!=speciesExpertSet.end();sIter++)
	...
	for(int clusterID=0;clusterID<maxClusterCnt;clusterID++)
	{
		...
		for(map<int,map<string,int>*>::iterator ogIter=allClusterAssignmentsGrouped.begin();ogIter!=allClusterAssignmentsGrouped.end();ogIter++)
		{
			...
			if(!toShowOG)
			{
				...
				for(map<int,map<string,int>*>::iterator hIter=geneAssignSet.begin();hIter!=geneAssignSet.end();hIter++)
				{
					map<string,int>* genes=hIter->second;
					genes->clear();
					delete genes;
				}
				geneAssignSet.clear();
				for(map<int,map<string,string>*>::iterator hIter=geneNamesSet.begin();hIter!=geneNamesSet.end();hIter++)
				{
					map<string,string>* genes=hIter->second;
					genes->clear();
					delete genes;
				}
				geneNamesSet.clear();
...
SpeciesClusterManager::dumpAllInferredClusterGammas(const char* outputDir,vector<string>& speciesList)
{
	...
	for(map<int,map<string,map<int,double>*>*>::iterator ogIter=mappedClusterGamma.begin();ogIter!=mappedClusterGamma.end();ogIter++)
	{
		...
		//SR notes: We don't need multiple gene sets for the cell types as this is automatically always 1 to 1
		//map<int,map<string,string>*>& geneSets=mgrp->getGeneSets();
		//for(map<int,map<string,string>*>::iterator hIter=geneSets.begin();hIter!=geneSets.end();hIter++)
		//{
		//	map<string,string>* gset=hIter->second;
		...
		//	oFile << gnm.getCommonName((*gset)[srcSpecies].c_str());
			const char* srcSpecGeneName=mgrp->getSpeciesHit(srcSpecies);
			if(srcSpecGeneName==NULL)
			{
				continue;
			}
			oFile << gnm.getCommonName(srcSpecGeneName);
			for(int s=0;s<speciesList.size();s++)
			{
				map<int,double>* geneclusterID=NULL;
				if(mgrp->getSpeciesHit(speciesList[s].c_str())==NULL)
				{
					geneclusterID=NULL;
				}
				else 
				{	
					const char* specHit=mgrp->getSpeciesHit(speciesList[s].c_str());
					string geneName(specHit);
					if(geneAssign.find(geneName)!=geneAssign.end())
					{
						geneclusterID=geneAssign[geneName];
					}
