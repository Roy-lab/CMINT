...
int
GeneTreeManager::clearTrees()
{
	for(map<int,GeneTree*>::iterator tIter=genetreeSet.begin();tIter!=genetreeSet.end();tIter++)
	{
		delete tIter->second;
	}
	genetreeSet.clear();
...
GeneTreeManager::createTree(SpeciesDistManager::Species* duplicateSpecies,SpeciesDistManager::Species* node, MappedOrthogroup* mor, int copyno)
{
	...
	if(node->children.empty())
	{
		...
		const char* geneName=mor->getSpeciesHit(node->name.c_str());
		if(geneName==NULL)
		{
			return NULL;
		}
	...
	//else
	{
		...
		{
		    geneTreeNode->name.append(geneName);
		}
