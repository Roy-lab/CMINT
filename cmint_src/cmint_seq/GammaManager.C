...
GammaManager::initGamma(int ogid, string& geneName, string& specName,int clustID)
{
	...
	if(gammaSet.find(ogid)==gammaSet.end())
	{
		...
		gamma->initUsingGeneTree(gtree,maxClusterCnt);
		/*if(gamma->getDupAncestor().length()>0)
		{
			ogDupAncMap[ogid]=gamma->getDupAncestor();
		}*/
		if(ogid==497718 || ogid==497719)
		{
			cout <<"Now showing " << ogid << endl;
			gamma->showTree();
			cout<<"Size of gamma" << sizeof(Gamma)<< endl;
		}
		delete gtree;
	}
	else
...
int 
GammaManager::setObservedProbs(int ogid,map<int,double>& prob,string&geneName,string& specName)
{
    Gamma* gamma_og=gammaSet[ogid];
    Gamma::Node* m=gamma_og->getGeneNode(geneName,specName);
    if(m->mixProbs==NULL)
    {
	m->mixProbs=new double[maxClusterCnt];
    }	
    for(map<int,double>::iterator pIter=prob.begin();pIter!=prob.end();pIter++)
    {
    	m->mixProbs[pIter->first]=pIter->second;
    }
    m->hasData=true;
...
int GammaManager::estimateLeafGamma(Gamma::Node* m)
{
	...
	for(int r=0;r<m->gamma->getRowCnt();r++)
	{
		...
		for(int cid=0;cid<maxClusterCnt;cid++)
		{
			if(prob==NULL)
			{
				cout <<"Oops! The value is null here for "<< m->name << endl;
			}
			double pval=prob[cid];//pIter->second;
			//double prior=conditional->getValue(r,pIter->first);
			double prior=conditional->getValue(r,cid);
			...
			if(isnan(post) || isinf(post))
			{
				...
				cout <<"Posterior is nan/inf for  "<< geneName <<" species " << specName << "cluster id "  << cid
				//cout <<"Posterior is nan/inf for  "<< geneName << "cluster id "  << pIter->first
				<< " pval=" << pval << " prior=" << prior << endl;
			}
			anormTerm=anormTerm+post;
			//m->gamma->setValue(post,r,pIter->first);
			m->gamma->setValue(post,r,cid);
		}
